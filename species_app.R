...
occurrences <- read.table("data/endemic_dolo50.txt", header = TRUE)
...
ui <- fluidPage(
  ...
  p("coordinates are centroids of a 50x50m grid cell"),
  # Compact search panel at the top
  fluidRow(
    column(
      width = 12,  # Full width for the search panel
      wellPanel(
  ...
  # Full-width map below, covering the screen
  fluidRow(
    column(
      width = 12,  # Map takes full width
      leafletOutput("map", height = "90vh")  # 90% of the viewport height for the map
    )
  )
)
...
server <- function(input, output, session) {
  observeEvent(input$update, {
    ...
    aoo_area <- aoo(coords)# Assuming a 2 km x 2 km grid for AOO
